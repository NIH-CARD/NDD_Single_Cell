...
for(CHROM in 1:21) {
    ...
    fwrite(chr_eQTL_snps, file=paste0('data/', CHROM, '_eQTL_SNPs.tsv'), row.names=F, col.names=T, sep='\t', quote=FALSE)
}

# Combining tables

dat <- foreach(file=list.files(path='data/', pattern='*_eQTL_SNPs.tsv', full.names=T), .combine='rbind') %do% {
    fread(file)
}

fwrite(dat, file='data/eQTL_colocalization_SNPs.tsv', quote=F, row.names=F, col.names=T, sep='\t')